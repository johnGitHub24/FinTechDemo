...
| 5173 | Vue Dev | 前端 SPA |
| 3000 | Grafana | 觀測畫圖（admin／admin；profile monitoring） |
| 9090 | Prometheus | 指標刮取／PromQL |
| 8089 | Locust | 壓測 UI（`run-loadtest.ps1 -WebUi`） |
...
4. `.\scripts\run-loadtest.ps1 -WebUi` → `:8089`（Users 5／Host `:8081`）
...
見藍圖頁 `#k8s-verify` 或 `docs/deploy/k8s-tips.md`。
