...
class RiskApiIntegrationTest {
    ...
    /**
     * CASE RISK-003：名義金額超過上限 → allowed=false（reason 含 max）。
     */
    @Test
    void RISK_003_overMaxNotional_shouldReject() throws Exception {
        mockMvc.perform(post("/api/risk/check")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(DemoTestFixtures.loadJson("risk", "RISK-003-REJECT")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.allowed").value(false))
                .andExpect(jsonPath("$.reason").value(org.hamcrest.Matchers.containsString("max")));
    }
}
