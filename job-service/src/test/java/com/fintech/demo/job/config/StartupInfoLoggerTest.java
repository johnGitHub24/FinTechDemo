...
class StartupInfoLoggerTest {
    ...
    void enabled_printsBannerWithoutProbe() {
        ...
        when(env.getProperty("startup.info.project-name", "FinTechDemo Job")).thenReturn("FinTechDemo Job");
        when(env.getProperty("server.port", "8083")).thenReturn("8083");
        ...
        when(env.getProperty("spring.datasource.url", "jdbc:h2:mem:job")).thenReturn("jdbc:h2:mem:job");
        ...
        assertThat(out).contains("FinTechDemo Job 後端已啟動");
        assertThat(out).contains("http://localhost:8083/actuator/health");
