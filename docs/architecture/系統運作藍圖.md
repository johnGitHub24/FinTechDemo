...
> **互動（Mermaid + 表）**：http://localhost:5173/blueprint#frontend-pages  
> **靜態 HTML**：[k8s-complete-guide.html §11](../guides/k8s-complete-guide.html#s11)

`/blueprint` **不是**獨立 `.html`：`index.html` → `App.vue` → Vue Router → `BlueprintView.vue`。

| URL | View | 守衛 |
|-----|------|------|
| `/login` | LoginView | 公開 |
| `/blueprint` | BlueprintView | 公開 |
| `/trade` | TradeView | JWT |
| `/portal` | PortalView | JWT |
| `/portal/audit` | AuditView | ADMIN |
| `/demo/*.html` | `public/demo/` | 不經 Router |

路由：`frontend/src/router/index.js` · 映像打包：藍圖 `#frontend-image`

---

## 0b. Docker／Redis

> **互動指令（複製鈕）**：頂欄 **Docker**／Demo 快捷 → **Docker** 分頁；藍圖亦有 http://localhost:5173/blueprint#docker-redis  
...
**誰連 Redis：**只有 **account-service** 連 `:6379`，寫 `account:{id}`／`positions:{id}`（TTL 600s）。**order／risk／gateway／job 本機預設不連 Redis**。`開啟Demo.cmd` 預設 `docker compose up -d redis`；沒開容器時 account 降級打 H2。
...
| Redis | starter-data-redis | 高速快取／暫存狀態 | Account cache-aside（本機 Demo 預設開；TTL 600s／入帳 evict） |
...
| Redis Cache | `account-service` `AccountQueryService` | cache-aside；TTL 600s；入帳 evict；只有 account 連 :6379 |
...
| 8082 | Risk | 名目風控（成交必開） |
| 8083 | Job | 逾時取消（可選） |
| 8084 | Account | 餘額／持倉／Redis Cache（本機預設開）／Kafka（可選） |
| 6379 | Redis | Account 快取（`開啟Demo` 預設 `docker compose up -d redis`） |
| 9093 | order H2 TCP | DataGrip：`jdbc:h2:tcp://localhost:9093/mem:fintechdemo` |
| 9094 | account H2 TCP | DataGrip：`jdbc:h2:tcp://localhost:9094/mem:accountdb` |
| 5173 | Vue Dev | 前端 SPA |
...
**NOTE 啟動（詳 [handbook §6.1](../portals/handbook.html#monitoring-start)）：**

1. 先 Docker Desktop Ready  
2. `.\開啟Demo.cmd`（內建 `Ensure-Monitoring`）或  
   `docker compose --profile monitoring up -d prometheus grafana`  
3. Grafana `:3000`（`admin`／`admin`）、Prometheus `:9090`  
4. 無 Docker 備援：`.\demo\start-monitoring-local.ps1`

設定檔：`docker-compose.yml`（profile `monitoring`）· `monitoring/prometheus.yml`（15s scrape `host.docker.internal:808x/actuator/prometheus`）·  
Grafana `provisioning/` + `dashboards/fintech-demo.json` · 各服務 Actuator 暴露 `prometheus`。

接著可：

1. Prometheus Graph：`up{job=~"fintech-.*"}`；Order RPS：`sum(rate(http_server_requests_seconds_count{job="fintech-order"}[1m]))`
2. Grafana → **FinTechDemo Overview**
3. Locust：見 `loadtest/README.md` → `:8089`
