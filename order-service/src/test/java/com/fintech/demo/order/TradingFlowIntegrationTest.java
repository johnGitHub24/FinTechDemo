 ...
 * 【技巧】以真實 Repository 與 MockMvc 跨越 HTTP、Security、Service 及 JPA 層；外部 Client 以 MockBean 隔離。
...
class TradingFlowIntegrationTest {
    ...
    /**
     * CASE-TRADING-FLOW-001：Given 種子資料已建立，When 查詢各 Repository，Then 資料關聯完整。
     */
    @Test
    void seedData_shouldLinkAllTables() {
        var trader = userRepository.findByUsername(DemoDataSeeder.TRADER1).orElseThrow();
    ...
    /**
     * CASE-TRADING-FLOW-002：Given 未帶 JWT，When 存取受保護帳戶 API，Then 回應 401 Unauthorized。
     */
    @Test
    void withoutToken_shouldGet401() throws Exception {
        mockMvc.perform(get("/api/accounts/me"))
                .andExpect(status().isUnauthorized());
    }

    /**
     * CASE-TRADING-FLOW-003：Given USER 與 ADMIN Token，When 查詢稽核 API，Then 僅 ADMIN 可取得資料。
     */
    @Test
    void userCannotAccessAudit_adminCan() throws Exception {
    ...
    /**
     * CASE-TRADING-FLOW-004：Given 已登入種子交易者，When 查詢入口 API，Then 回傳帳戶、持倉、訂單與市場資料。
     */
    @Test
    void portalApis_shouldReflectSeededTrader() throws Exception {
    ...
    /**
     * CASE-TRADING-FLOW-005：Given 風控核准的買單，When 建單並執行，Then 訂單、餘額與持倉同步更新。
     */
    @Test
    void frontToBack_createExecute_shouldUpdateHistoryBalancePosition() throws Exception {
    ...
    /**
     * CASE-TRADING-FLOW-006：Given 新建待處理訂單，When 取消訂單，Then 歷程保存 CANCELLED 狀態。
     */
    @Test
    void cancelPending_shouldAppearInHistory() throws Exception {
        String body = """
                {
    ...
    /**
     * CASE-TRADING-FLOW-007：Given 已授權管理者，When 查詢訂單列表，Then 可取得全體訂單。
     */
    @Test
    void adminList_shouldSeeAllOrders() throws Exception {
