...
| 角色分流 | USER：自己的交易與查詢；ADMIN：B1／B2 仍是**本人帳戶**，B3／B5 才是**全站訂單**（畫面須標示擁有者，避免把 trader1 成交當成 admin 該有持倉） |
...
| 概念 | 規則（精簡） |
|------|----------------|
| 種子用戶 | `trader1`（USER）：種子成交後現金 **85,000**＋持倉 AAPL 100@150；`admin`（ADMIN）：現金 **100,000**、**無持倉** |
